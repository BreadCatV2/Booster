import { z } from "zod";
import { db } from "@/db";
import { comments, userFollows, users, videoRatings, videos, videoUpdateSchema, videoViews } from "@/db/schema";
import { createTRPCRouter, protectedProcedure, baseProcedure } from "@/trpc/init";
import {  eq, and, getTableColumns, sum, avg, inArray, isNotNull, sql, count, desc, lt, not, or } from "drizzle-orm";
...
export const videosRouter = createTRPCRouter({
    ...
    getMore: baseProcedure
        ...
        .query(async ({ ctx, input }) => {
            ...
            const whereParts: any[] = [and(eq(videos.visibility, "public"), not(eq(videos.status, "processing")), not(eq(videos.id,videoId)))]
