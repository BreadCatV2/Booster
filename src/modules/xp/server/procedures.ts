...
export const xpRouter = createTRPCRouter({
    ...
    .mutation(async ({ctx,input}) => {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: `${process.env.VERCEL_URL}/market?status=success`,//`${process.env.VERCEL_URL ?? "http://localhost:3000"}/market?status=success`,
        cancel_url: `${process.env.VERCEL_URL}/market?status=success`,//`${process.env.VERCEL_URL} ?? "http://localhost:3000"}/market?status=cancel`,
